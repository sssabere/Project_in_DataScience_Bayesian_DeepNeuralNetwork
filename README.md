...
The data required to run [Combine_CNN_BLR.ipynb](./Combine_CNN_BLR.ipynb) is not included in this repository but it can be downloaded shared Drive folder.
Follow these steps:

1. Download the data file from [HERE](https://drive.google.com/drive/folders/1pcT3Md9fMlrWPXrMy8W_DS72sEvPCarS?usp=drive_link) which is a shared Drive folder called “Project in DS”.
Inside it, you’ll find the dataset required for Combine_CNN_BLR.ipynb.

2. Place the data file inside your repository directory to make sure the notebook’s file paths match the location the data.
...
- Training dataset is split into two parts:  
  - 90% of the training set is used for training individual models (CNN and Bayes) and the remaining 10% is used for validatin purposes. 
    - CNN is trained with 85% dropout rate, while Bayes is trained with 15% dropout.  
  - All models are tested on the test set that is given. For the combined model we degrade this test set on different levels and we test the combined model based on this. 
